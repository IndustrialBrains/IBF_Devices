...
class Tests(unittest.TestCase):
    ...
    def test_01_device_added(self) -> None:
        device_name = conn.read_by_name(f"{self.PREFIX_DEVICE}.stIdentity.sName")
    ...
    def test_04_block_manual(self) -> None:
        self._enable()
        conn.write_by_name(f"{self.PREFIX_CONTROLLER}.bBlockManualMode", True)
        self.assertTrue(wait_value(f"{self.PREFIX_DEVICE}.bManual", False, 1))
